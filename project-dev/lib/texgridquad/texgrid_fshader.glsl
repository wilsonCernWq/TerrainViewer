...
void main() {
    ...
    // shadow
    float shadow = 1.0;
    float bias = 0.002;//clamp(0.005*tan(acos(cosTheta)), 0.0, 0.002);
    for (int i=0; i < 16;i++){
    ...
    // calculate filter
    float h = height;
    float fg = 0.5 * rfit(h,0.5,0.30,0.1) * alpha(n, 0.6, 0.1);
    float ff = 0.5 * rfit(h,0.3,0.15,0.1) * alpha(n, 0.6, 0.1);
    float fw = 0.5 * rfit(h,0.4,0.30,0.1) * alpha(n, 0.6, 0.1);
    float fh = clamp(hfit(h,0.6,0.1)+rfit(h,0.6,0.4,0.1)*alpha(n,0.6,0.1),0.0,1.0);
    float fl = lfit(h,0.2,0.02);

    // rock and glass
    color = mix(texture(color_tex1, 20.0 * uv).rgb, texture(color_tex0, 50.0 * uv).rgb, fg);
    color = mix(color, texture(color_tex4, 50.0 * uv).rgb, ff);
    color = mix(color, texture(color_tex5, 50.0 * uv).rgb, fw);
    // snow
    color = mix(color, texture(color_tex2, 30.0 * uv).rgb, fh);
    // sand
    color = mix(color, texture(color_tex3, 60.0 * uv).rgb, fl);

    color = mix(color, color * Id * shadow * diffusion, 0.7);
}
