...
public:
    ...
    void setWaterHeight(float *height) {
        terrainGrid.setWaterHeight(height);
    }
    ...
    float getTerrainHeight(float x, float y, float sfactor) {
        ...
        if (-1.0 <= x && x < 1.0 && -1.0 <= y && y < 1.0) {
            ...
            float height = h * maxHeight / displayRatio;
            height = height > 0.2 ? height : 0.2;
    ...
    void draw(mat4& M, mat4& view, mat4& proj, mat4& trackball, vec2 origin, bool inverted = false) {
        terrainGrid.draw(trackball, M, view, proj, ligproj * ligview, origin, inverted);
