...
void resetcam() {
    glob::campos = vec3(0.0, 6.0, 10.0);
    glob::camdir = vec3(0.0, 0.0, -1.0);
    glob::camupv = vec3(0.0, 1.0,  0.0);
    time_prev = 0.0;
    move_velo = 0.0;
    trackball.matrix()=mat4::Identity();
}
...
void move_camera(NAVIGATION_MODE mode) {
    ...
    if (mode == TRACKBALL) {
        if (glfwGetKey((int)'W') == GLFW_PRESS) {
            ...
            vec3 dir = vec3(camdir(0), 0.0, camdir(2));
            campos += dir * move_velo * dt;
        }
